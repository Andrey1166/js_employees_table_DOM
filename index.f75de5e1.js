...
thead.addEventListener("click", (e)=>{
    ...
    function asc() {
        return [
            ...
            else return item1.cells[index].innerText.localeCompare(item2.cells[index].innerText);
        });
    }
    // desc sorted list
    function desc() {
        return [
            ...
            else return item2.cells[index].innerText.localeCompare(item1.cells[index].innerText);
        });
    }
...
table.tBodies[0].addEventListener("click", (e)=>{
    ...
    [
        ...
        if (row === tr && !row.classList.contains("active")) row.classList.add("active");
...
form.addEventListener("submit", (e)=>{
    e.preventDefault();
    const nameInput = document.querySelector('[name="name"]').value.trim().replace(/[^a-zA-Z]/g, "");
    const position = document.querySelector('[name="position"]').value.trim();
    const age = document.querySelector('[name="age"]').value.trim();
    const office = document.querySelector('[name="office"]').value.replace(/-/g, " ").split(" ").map((word)=>`${word.charAt(0).toUpperCase() + word.slice(1)}`).join(" ");
    const salary = document.querySelector('[name="salary"]').value.trim();
    const row = document.createElement("tr");
    if (nameInput.length < 4) {
        pushNotification(10, 10, "Error", "Name must be longer than 4 characters!!!", "error");
        return;
    }
    if (+age < 18 || +age > 90) {
        pushNotification(10, 10, "Error", "Age must be more than 18 and less than 90!!!", "error");
        return;
    }
    if (!position || !office || !salary) {
        pushNotification(10, 10, "Error", "Please fill out all fields with valid values!!!", "error");
        return;
    }
    if (Number.isNaN(+salary) || !Number.isFinite(+salary)) {
        pushNotification(10, 10, "Error", "Please fill out salary field with valid value!!!", "error");
        return;
    }
...
table.tBodies[0].addEventListener("dblclick", (e)=>{
    ...
    inputTd.addEventListener("blur", ()=>{
        if (inputTd.value.length > 0) td.textContent = inputTd.value;
        else td.textContent = text;
    });
    inputTd.addEventListener("keydown", (ev)=>{
        if (ev.key === "Enter") inputTd.blur();
    });
});
